...
const minimumTtl=Math.max(300,Number(process.env.FOMO_LOCAL_SYNC_MIN_TOKEN_TTL_SECONDS??1800));
...
const waitForFreshToken=async()=>{
  ...
  while(Date.now()<deadline){const expiry=tokenExpiry(readStoredFomoToken(tokenPath));if(expiry&&expiry-Date.now()/1000>=minimumTtl)return expiry;await sleep(1000);}
