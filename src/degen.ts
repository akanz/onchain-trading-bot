...
const signalLabel = (type: number) =>
  type === 6 ? "PRICE SURGE" : type === 7 ? "NEW ATH" : type === 12 ? "SMART MONEY" : undefined;

export function buildDegenRows(
  chain: Chain,
  trending: Json[],
  signals: Json[],
  tokenConfig: Json,
  ...
  const merge = (market: Json, source: string, microcapOnly = false) => {
    ...
    if (microcapOnly && (marketCap === undefined || marketCap <= 0 || marketCap > maxMarketCap))
      return;
  ...
  for (const row of signals) {
    const type = Number(row.signal_type),
      label = signalLabel(type),
      triggerAt = Number(row.trigger_at ?? 0);
    if (
      !label ||
      !triggerAt ||
      triggerAt < now - Number(process.env.SIGNAL_LOOKBACK_SECONDS ?? 1800)
    )
      continue;
    const market: Json = {
      ...marketSnapshot(row),
      ...
      trigger_mc: finite(row.trigger_mc),
      signal_times: finite(row.signal_times),
    };
    ...
    if (merged && !merged.degen_signal_labels.includes(label))
      merged.degen_signal_labels.push(label);
  }
      ...
      row.degen_signal_labels?.includes("PRICE SURGE")
        ...
        : row.degen_signal_labels?.includes("NEW ATH")
          ...
          : row.degen_signal_labels?.includes("SMART MONEY")
            ? 2
            : 1;
    const momentum = (row: Json) =>
      finite(row.price_change_percent5m ?? row.price_change_percent) ?? -Infinity;
    const turnover = (row: Json) => {
      const volume = finite(row.volume) ?? 0,
        mc = finite(row.market_cap) ?? 1;
      return volume / mc;
    };
    return (
      Number(b.is_microcap) - Number(a.is_microcap) ||
      priority(b) - priority(a) ||
      momentum(b) - momentum(a) ||
      turnover(b) - turnover(a) ||
      (finite(b.volume) ?? 0) - (finite(a.volume) ?? 0)
    );
  });
}
