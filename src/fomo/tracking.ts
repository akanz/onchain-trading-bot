...
export function buildFomoTrackedWallets(profiles:Json[],tokens:FomoToken[],enabledChains:Chain[],minimumTrendingAthMarketCap=1_000_000):Json[] {
  const minimumTrendingRunnerChangePercent=configured("FOMO_TRENDING_RUNNER_MIN_CHANGE_PERCENT",300);
  ...
  for(const token of tokens){
    const sources=[] as string[];if(token.sources.has("fomo_most_held"))sources.push("fomo_most_held_500pct");if(token.sources.has("fomo_trending")&&(crossedHistoricalMarketCap(token,minimumTrendingAthMarketCap)||Number(token.change24??0)*100>=minimumTrendingRunnerChangePercent))sources.push("fomo_trending_500pct");if(!sources.length)continue;
