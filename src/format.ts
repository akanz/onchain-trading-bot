...
export function formatTrackingStatus(chains:Chain[],diagnostics:Json):string {
  ...
  for(const chain of chains){
    const row=diagnostics[chain]??{},buys=Array.isArray(row.fomo_recent_buys)?row.fomo_recent_buys:[],sells=Array.isArray(row.fomo_recent_sells)?row.fomo_recent_sells:[],safety=row.tracked_buy_safety??{},poll=row.tracked_wallet_poll??{},multiples=row.multiplier_monitor??{},surges=row.surge_wallet_attribution??{};
    lines.push(`\n<b>${escapeHtml(chain.toUpperCase())}</b>\nMonitored addresses: ${Number(row.gmgn_tracked_wallets??0)} · Fomo-native profiles: ${Number(row.fomo_tracked_profiles??0)}\nWallet poll: ${Number(poll.attempted_wallets??0)} addresses · ${Number(poll.events??0)} recent events${poll.rate_limited?" · paused by rate limit":""} · ${escapeHtml(poll.refreshed_at??"not completed")}\n2h multiplier baselines: ${Number(multiples.active_baselines??0)} active · ${Number(multiples.crossed_unannounced??0)} crossed but unannounced · max ${Number(multiples.max_observed_multiple??1).toFixed(2)}×\nSurge attribution: ${Number(surges.tokens_checked??0)} checked · ${Number(surges.surge_events??0)} moves · ${Number(surges.confirmed_profitable_wallets??0)} confirmed ≥300% · ${Number(surges.track_worthy_wallets??0)} track-worthy${surges.rate_limited?" · paused by rate limit":""}\nRecent Fomo activity: ${Number(row.fomo_recent_buy_swaps??0)} buys / ${Number(row.fomo_recent_sell_swaps??0)} sells · profile errors: ${Number(row.fomo_profile_scan_errors??0)}\nSafety checks: ${Number(safety.checked??0)} · passed: ${Number(safety.passed??0)} · suppressed: ${Number(safety.suppressed??0)} · unavailable: ${Number(safety.unavailable??0)}`);
    const audit=row.discovery_audit??{};lines.push(`DexScreener: ${Number(row.dexscreener_discovered??0)} discovered · ${Number(row.dexscreener_candidates??0)} movement candidates · ${escapeHtml(row.dexscreener_error?`error: ${row.dexscreener_error}`:row.dexscreener_refreshed_at??"not completed")}`);lines.push(`Discovery audit: ${Number(audit.passed??0)} passed · ${Number(audit.suppressed??0)} suppressed · ${Number(audit.pending??0)} pending`);
    if(chain==="robinhood")lines.push(`Long launchpad: ${Number(row.long_assets??0)} assets · ${Number(row.long_candidates??0)} recent candidates · ${escapeHtml(row.long_error?`error: ${row.long_error}`:row.long_refreshed_at??"not completed")}`);
    for(const buy of buys.slice(0,3)){const amount=Number(buy.amount_usd),amountText=Number.isFinite(amount)&&amount>0?` · ${money(amount)}`:"",when=buy.bought_at?` · ${escapeHtml(buy.bought_at)}`:"";lines.push(`• <b>${traderName(buy.trader)}</b> bought <code>${escapeHtml(buy.token)}</code>${amountText}${when}`);}
    for(const sell of sells.slice(0,3)){const amount=Number(sell.amount_usd),amountText=Number.isFinite(amount)&&amount>0?` · ${money(amount)}`:"",when=sell.sold_at?` · ${escapeHtml(sell.sold_at)}`:"";lines.push(`• <b>${traderName(sell.trader)}</b> sold <code>${escapeHtml(sell.token)}</code>${amountText}${when}`);}
  }
...
export function formatSuppressedTokens(chains:Chain[],diagnostics:Json):string[] {
  const flagText=(value:unknown)=>value===true||value===1||value==="1"||value==="yes"?"yes":value===false||value===0||value==="0"||value==="no"?"no":"unknown",messages:string[]=[],trackedKeys=new Set<string>();
  for(const chain of chains){const rows=Array.isArray(diagnostics[chain]?.tracked_buy_safety?.recent_suppressed)?diagnostics[chain].tracked_buy_safety.recent_suppressed:[];for(const row of rows){trackedKeys.add(`${chain}:${String(row.address).toLowerCase()}`);const n=(value:unknown)=>Number.isFinite(Number(value))?Number(value):undefined,traders=(row.traders??[]).map(traderName).filter(Boolean),reason=(row.reasons??[]).map((value:unknown)=>escapeHtml(String(value).replace(/^FAIL\s+/,""))).join("; "),gmgn=httpUrl(row.gmgn_url),boughtAt=n(row.bought_at);messages.push([
  ...
  for(const chain of chains){const rows=Array.isArray(diagnostics[chain]?.discovery_audit?.recent_suppressed)?diagnostics[chain].discovery_audit.recent_suppressed:[];for(const row of rows){if(trackedKeys.has(`${chain}:${String(row.address).toLowerCase()}`))continue;const n=(value:unknown)=>Number.isFinite(Number(value))?Number(value):undefined,reasons=(row.reasons??[]).map((value:unknown)=>String(value).replace(/^FAIL\s+/,"")),rug=explicitRugText(reasons),url=httpUrl(row.dexscreener_url);messages.push([`${rug?"⚠️":"🛡️"} <b>SUPPRESSED DISCOVERY · ${escapeHtml(chain.toUpperCase())}</b>`,`<b>${escapeHtml(row.symbol??row.name??"Unknown")}</b>`,`🔍 Found via ${(row.sources??[]).map((value:unknown)=>escapeHtml(value)).join(" · ")||"market discovery"}${(row.signals??[]).length?` · ${(row.signals??[]).map((value:unknown)=>escapeHtml(value)).join(" · ")}`:""}`,`💰 Price ${price(n(row.price))} · MC ${money(n(row.market_cap))} · liquidity ${money(n(row.liquidity))}`,`📊 Volume 5m ${money(n(row.volume_5m))} · 5m ${percent(n(row.price_change_5m))} · holders ${compact(n(row.holder_count))} · Top 10 ${rate(n(row.top_10_holder_rate))}`,`🛡️ Honeypot ${flagText(row.honeypot)} · source verified ${flagText(row.open_source)} · owner renounced ${flagText(row.renounced)} · LP locked ${flagText(row.liquidity_locked)}`,`${rug?"⚠️ Rug-risk evidence":"🚫 Blocked because"}: ${reasons.map(escapeHtml).join("; ")||"safety screen did not pass"}`,`<code>${escapeHtml(row.address)}</code>${url?`\n${link("DexScreener",url)}`:""}`].join("\n").slice(0,4050));}}
  return messages.length?messages:[`🔎 No recently suppressed tracked-buy or discovery tokens for ${escapeHtml(chains.map(chain=>chain.toUpperCase()).join(" · "))}.`];
}
...
function formatDiscoveryFeed(chains:Chain[],rows:Json[],kind:"surged"|"potential"):string[]{
  if(!rows.length)return [];
  const titleText=kind==="surged"?"SURGED TOKENS":"SERIOUS POTENTIAL RUNNERS",icon=kind==="surged"?"⚡":"🌱",chunks:Json[][]=[];
  for(let index=0;index<rows.length;index+=10)chunks.push(rows.slice(index,index+10));
  return chunks.map((items,chunkIndex)=>{const lines=[`${icon} <b>${titleText}</b>`,`${escapeHtml(chains.map(chain=>chain.toUpperCase()).join(" · "))} · Robinhood → BSC → Solana priority · ${chunkIndex+1}/${chunks.length}`];for(const [offset,row] of items.entries()){const n=(value:unknown)=>Number.isFinite(Number(value))?Number(value):undefined,symbol=escapeHtml(row.symbol??row.name??"Unknown"),labels=[...(row.degen_signal_labels??[]),...(row.trending_signal_tags??[])].map((value:unknown)=>escapeHtml(value)).filter(Boolean),change=n(row.price_change_5m??row.price_change_percent5m??row.price_change_percent),score=n(row.potential_runner_score);lines.push(`\n${chunkIndex*10+offset+1}. <b>${escapeHtml(String(row.chain??"?").toUpperCase())} · $${symbol}</b>${score===undefined?"":` · potential ${Math.round(score)}/100`}`);lines.push(`💎 MC ${money(n(row.market_cap))} · Liq ${money(n(row.liquidity))} · Vol ${money(n(row.volume_5m??row.volume??row.volume_1h))} · 5m ${percent(change)}`);if(labels.length)lines.push(`🔔 ${labels.slice(0,4).join(" · ")}`);if(row.surge_attribution?.wallets?.length)lines.push(`👤 Profitable pre-move wallets: ${Number(row.surge_attribution.wallets.length)} · track-worthy ${Number(row.surge_attribution.track_worthy_wallets??0)}`);if(row.address)lines.push(`<code>${escapeHtml(row.address)}</code>`);}lines.push(kind==="surged"?"\nℹ️ A surge is a confirmed price/volume event, not an instruction to chase.":"\nℹ️ Potential runners passed the early safety screen and have pre-run evidence such as launch progress, smart money, distributed volume, or holder growth. Recheck immediately before acting.");return lines.join("\n").slice(0,4050);});
}

export function formatSurgedDigest(chains:Chain[],rows:Json[]):string[]{return formatDiscoveryFeed(chains,rows,"surged");}
export function formatPotentialDigest(chains:Chain[],rows:Json[]):string[]{return formatDiscoveryFeed(chains,rows,"potential");}
...
function formatPonsDegenCard(row:Json):string {
  ...
  if(row.surge_attribution?.event){const event=row.surge_attribution.event,wallets=Array.isArray(row.surge_attribution.wallets)?row.surge_attribution.wallets:[];lines.push(`⚡ ${escapeHtml(event.kind??"SURGE")} · ${percent(n(event.price_change_percent))} · ${Number(event.volume_ratio??0).toFixed(1)}× volume · ≥300% wallets ${wallets.length}`);for(const wallet of wallets.slice(0,2))lines.push(`• <b>${wallet.worth_tracking?"TRACK":"OBSERVE"}</b> ${traderName(wallet.name??wallet.wallet)} · position ${percent(n(wallet.position_roi_percent))} · 30d ${percent(n(wallet.realized_roi_30d_percent))} · all ${percent(n(wallet.realized_roi_all_percent))}`);}
...
export function formatTokenCard(s:TokenSnapshot,context:TokenCardContext={}):string {
  ...
  if(context.surgeAttribution?.event){const numeric=(value:unknown)=>Number.isFinite(Number(value))?Number(value):undefined,event=context.surgeAttribution.event,wallets=Array.isArray(context.surgeAttribution.wallets)?context.surgeAttribution.wallets:[];lines.push(`⚡ <b>${escapeHtml(event.kind??"SURGE")} ATTRIBUTION</b> · ${percent(numeric(event.price_change_percent))} · volume ${money(numeric(event.volume_usd))} · ${Number(event.volume_ratio??0).toFixed(1)}× normal`);lines.push(`🔍 Profitable pre-move wallets: ${wallets.length} · track-worthy after 30d/all-time checks: ${Number(context.surgeAttribution.track_worthy_wallets??0)}`);for(const wallet of wallets.slice(0,3)){const label=wallet.name?escapeHtml(wallet.name):escapeHtml(short(String(wallet.wallet))),status=wallet.worth_tracking?"TRACK":"OBSERVE";lines.push(`• <b>${status}</b> ${label} · position ROI ${percent(numeric(wallet.position_roi_percent))} · 30d ${percent(numeric(wallet.realized_roi_30d_percent))} · all ${percent(numeric(wallet.realized_roi_all_percent))}`);}}
...
export function formatAlert(a:Alert):string {
  ...
  if(a.token_snapshot)return formatTokenCard(a.token_snapshot,{tier:a.tier,sources:a.sources,signalStrength:a.signal_strength,walletCount:a.wallet_count,aggregateBuyUsd:a.aggregate_buy_usd,twitterAccounts:a.twitter_accounts,surgeAttribution:a.surge_attribution});
