...
export function formatTrackingStatus(chains:Chain[],diagnostics:Json):string {
  ...
  for(const chain of chains){const row=diagnostics[chain]??{},buys=Array.isArray(row.fomo_recent_buys)?row.fomo_recent_buys:[],sells=Array.isArray(row.fomo_recent_sells)?row.fomo_recent_sells:[],safety=row.tracked_buy_safety??{},multiples=row.multiplier_monitor??{},surges=row.surge_wallet_attribution??{};lines.push(`\n<b>${escapeHtml(chain.toUpperCase())}</b>\nMonitored addresses: ${Number(row.gmgn_tracked_wallets??0)} · Fomo-native profiles: ${Number(row.fomo_tracked_profiles??0)}\n2h multiplier baselines: ${Number(multiples.active_baselines??0)} active · ${Number(multiples.crossed_unannounced??0)} crossed but unannounced · max ${Number(multiples.max_observed_multiple??1).toFixed(2)}×\nSurge attribution: ${Number(surges.tokens_checked??0)} checked · ${Number(surges.surge_events??0)} moves · ${Number(surges.confirmed_profitable_wallets??0)} confirmed ≥300% · ${Number(surges.track_worthy_wallets??0)} track-worthy${surges.rate_limited?" · paused by rate limit":""}\nRecent Fomo activity: ${Number(row.fomo_recent_buy_swaps??0)} buys / ${Number(row.fomo_recent_sell_swaps??0)} sells · profile errors: ${Number(row.fomo_profile_scan_errors??0)}\nSafety checks: ${Number(safety.checked??0)} · passed: ${Number(safety.passed??0)} · suppressed: ${Number(safety.suppressed??0)} · unavailable: ${Number(safety.unavailable??0)}`);for(const buy of buys.slice(0,3)){const amount=Number(buy.amount_usd),amountText=Number.isFinite(amount)&&amount>0?` · ${money(amount)}`:"",when=buy.bought_at?` · ${escapeHtml(buy.bought_at)}`:"";lines.push(`• <b>${traderName(buy.trader)}</b> bought <code>${escapeHtml(buy.token)}</code>${amountText}${when}`);}for(const sell of sells.slice(0,3)){const amount=Number(sell.amount_usd),amountText=Number.isFinite(amount)&&amount>0?` · ${money(amount)}`:"",when=sell.sold_at?` · ${escapeHtml(sell.sold_at)}`:"";lines.push(`• <b>${traderName(sell.trader)}</b> sold <code>${escapeHtml(sell.token)}</code>${amountText}${when}`);}}
