...
export function gmgnTraderRoiEvidence(row: Json): Json {
  const percent = (value: unknown) => {
    ...
    return ratio === null ? undefined : ratio * 100;
  };
  const realized = percent(row.realized_pnl),
    unrealized = percent(row.unrealized_pnl),
    total = percent(row.profit_change);
  return {
    realized_roi_percent: realized,
    unrealized_roi_percent: unrealized,
    total_roi_percent: total,
    maximum_roi_percent: Math.max(
      ...[realized, unrealized, total].filter((value): value is number => value !== undefined),
    ),
...
export function gmgnTraderTags(row: Json): string[] {
  return [...(row.tags ?? []), ...(row.maker_token_tags ?? [])].map((tag: unknown) =>
...
export function isCleanGmgnTrendingTrader(
  ...
  const profit = Math.max(
    number(row.realized_profit, 0) ?? 0,
    number(row.unrealized_profit, 0) ?? 0,
    number(row.profit, 0) ?? 0,
  );
  if (profit < 100 || gmgnTraderRoiPercent(row) < minimumRoiPercent) return false;
  if ((number(row.sell_tx_count_cur, 0) ?? 0) < 1 || (number(row.buy_tx_count_cur, 0) ?? 0) > 50)
    return false;
  return true;
}

export function buildEliteGmgnWalletsFromTokenResults(
  results: Json[],
  minimumRoiPercent = Number(process.env.GMGN_MIN_TRENDING_TRADER_ROI_PERCENT ?? 500),
): Json[] {
  ...
  for (const result of results) {
    ...
    for (const trader of result.traders ?? []) {
      ...
      const wallet = String(trader.address),
        key = `${chain}:${wallet.toLowerCase()}`,
        evidence = gmgnTraderRoiEvidence(trader),
        row = wallets.get(key) ?? {
          chain,
          wallet,
          source: "gmgn",
          ...
          discovery_sources: ["gmgn_trending_500pct"],
          tokens: [],
          max_position_roi_percent: 0,
          max_realized_position_roi_percent: 0,
          max_unrealized_position_roi_percent: 0,
        };
      row.tokens.push({ address: token.address, symbol: token.symbol, ...evidence });
      row.max_position_roi_percent = Math.max(
        Number(row.max_position_roi_percent ?? 0),
        Number(evidence.maximum_roi_percent ?? 0),
      );
      row.max_realized_position_roi_percent = Math.max(
        Number(row.max_realized_position_roi_percent ?? 0),
        Number(evidence.realized_roi_percent ?? 0),
      );
      row.max_unrealized_position_roi_percent = Math.max(
        Number(row.max_unrealized_position_roi_percent ?? 0),
        Number(evidence.unrealized_roi_percent ?? 0),
      );
      wallets.set(key, row);
    }
  }
