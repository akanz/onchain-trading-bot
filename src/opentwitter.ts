...
export class OpenTwitterClient {
  ...
  async userTweets(username:string,maxResults=10):Promise<Json[]> {
    const payload=await this.post("/open/twitter_user_tweets",{username,maxResults,product:"Latest",includeReplies:false,includeRetweets:false});
    if(Array.isArray(payload))return payload;
    for(const value of [payload?.data,payload?.tweets,payload?.data?.tweets,payload?.result])if(Array.isArray(value))return value;
    return [];
  }
}
