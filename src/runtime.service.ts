...
export class RuntimeService implements OnModuleInit, OnModuleDestroy {
  ...
  get scheduledChains(): Chain[] {
    const names = new Set(
      (process.env.SCHEDULED_CHAINS ?? "robinhood,bsc,sol")
        .split(",")
  ...
  async onModuleInit(): Promise<void> {
    ...
    const mongo = await initializeMongo(
      this.connection.getClient() as unknown as MongoClient,
      this.connection.db as unknown as Db,
    );
    this.mongo = mongo;
    this.tracker = new TrackerService(this.config, undefined, undefined, undefined, mongo);
    this.botStore = new BotStore(mongo);
    await Promise.all([this.tracker.init(), this.botStore.init()]);
    if (!this.scheduledChains.length)
      throw new Error("SCHEDULED_CHAINS does not contain any enabled chain");
