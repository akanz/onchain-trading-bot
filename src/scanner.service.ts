...
export class ScannerService implements OnApplicationBootstrap,OnApplicationShutdown {
  ...
  async scanAndPublish():Promise<void>{
    ...
    try {
      ...
      const gmgnAvailable=!tracker.gmgn.cooldownUntil,trending=completedChains.length?await tracker.latestTrendingAcross(completedChains,Number(process.env.TRENDING_DIGEST_LIMIT??10)):[],rankedDegen=gmgnAvailable&&process.env.DEGEN_MODE==="true"&&completedChains.length?tracker.latestDegenAcross(completedChains,Number(process.env.DEGEN_DIGEST_LIMIT??20)):[],degen=rankedDegen.length?await tracker.enrichDegenRows(rankedDegen):[],multipleAlerts=gmgnAvailable&&completedChains.length?await tracker.monitorCallMultiples(completedChains,[...trending,...degen]):[];
      const multipleDelivery=await this.publishMultipleAlerts(multipleAlerts),trendingDelivery=completedChains.length&&trending.length?await this.telegram.trending(completedChains,trending):{attempted:0,sent:0,failed:0},degenDelivery=gmgnAvailable&&process.env.DEGEN_MODE==="true"&&completedChains.length?await this.telegram.degen(completedChains,degen):{attempted:0,sent:0,failed:0};if(trendingDelivery.sent>0)tracker.acknowledgeTrending(trending);
