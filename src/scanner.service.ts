...
export class ScannerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    this.addInterval("signal-scan", Math.max(Number(process.env.SCAN_INTERVAL_MS ?? 300000), 30000), () => this.runTracked(() => this.scanAndPublish()));
    ...
    this.runTracked(() => this.scanAndPublish()); this.runTracked(() => this.walletRefreshCheck());
  }
  ...
  async scanAndPublish(): Promise<void> {
    if (this.shuttingDown || this.scanning || this.walletRefreshRunning || this.multipleMonitoring) return;
  ...
  private async pollTriggeredMultiples(): Promise<void> {
    const tracker = this.runtime.tracker; if (this.shuttingDown || process.env.MULTIPLE_MONITOR_ENABLED === "false" || this.scanning || this.walletRefreshRunning || this.multipleMonitoring || tracker.gmgn.cooldownUntil) return; this.multipleMonitoring = true;
  ...
  private async walletRefreshCheck(): Promise<void> {
    if (this.shuttingDown || process.env.DAILY_WALLET_REFRESH_ENABLED === "false" || this.scanning || this.walletRefreshRunning || this.multipleMonitoring) return;
  ...
  async onApplicationShutdown(): Promise<void> {
    ...
    for (const name of ["signal-scan", "multiple-monitor", "mongo-roster-reload", "wallet-refresh-check", "fomo-session-health"]) try { this.scheduler.deleteInterval(name); } catch { }
