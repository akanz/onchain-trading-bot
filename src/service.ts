...
export class TrackerService {
  ...
  roster(chain:Chain){const stored=this.store(chain).roster();if(stored.length)return stored;return [...new Map(this.mongoTrackedWalletRows.filter(row=>row.chain===chain).map(row=>[String(row.wallet).toLowerCase(),row])).values()].sort((a,b)=>Number(b.score??b.wilson_closed_sample??b.wilson_30d??0)-Number(a.score??a.wilson_closed_sample??a.wilson_30d??0)).map(row=>({wallet:row.wallet,score:row.score??row.wilson_closed_sample??row.wilson_30d??0,assessed_at:row.generated_at??row.updated_at,source:row.source,tracking_tier:row.tracking_tier??"qualified",fomo_handle:row.fomo_handle}));}
  ...
  private loadTrackedWallets():Map<Chain,string[]> {
    ...
    const paths=[gmgnPath,fomoPath].filter((path):path is string=>typeof path==="string").filter(path=>existsSync(path));
