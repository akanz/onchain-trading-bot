...
import { number, scoreToken } from "./scoring.js";
import { OpenTwitterClient } from "./opentwitter.js";
import { FomoClient, fomoChain, type FomoToken } from "./fomo/client.js";
import { addressKey, extractContractAddresses, hasTrackedBuyCluster, marketSnapshot, passesMarketGate, shouldInvestigate, signalStrength, validTokenAddress, type SignalCandidate, type SignalSource } from "./signals.js";
...
export class TrackerService {
  ...
  roster(chain:Chain){const stored=this.store(chain).roster(),seeded=trackedWalletSeeds().filter(row=>row.chain===chain),fallback=[...this.mongoTrackedWalletRows.filter(row=>row.chain===chain),...seeded],combined=new Map<string,Json>();for(const row of [...stored,...fallback])combined.set(`${row.source??"stored"}:${String(row.wallet).toLowerCase()}`,row);return [...combined.values()].sort((a,b)=>Number(b.score??b.wilson_closed_sample??b.wilson_30d??0)-Number(a.score??a.wilson_closed_sample??a.wilson_30d??0)).map(row=>({wallet:row.wallet,score:row.score??row.wilson_closed_sample??row.wilson_30d??0,assessed_at:row.generated_at??row.updated_at,source:row.source,tracking_tier:row.tracking_tier??"qualified",fomo_handle:row.fomo_handle,qualifying_positions:row.qualifying_positions??[]}));}
  ...
  diagnostics(chains:Chain[]):Json {
    ...
    return Object.fromEntries(chains.map(chain=>{
      const fomoActivity=this.fomoActivity.get(chain)??[],clusters=new Map<string,Set<string>>();
      for(const row of fomoActivity){if(!recent(row,envNumber("SIGNAL_LOOKBACK_SECONDS",1800)))continue;const address=addressKey(String(row.tokenAddress??"")),wallet=addressKey(String(row.wallet??""));if(!address||!wallet)continue;const wallets=clusters.get(address)??new Set<string>();wallets.add(wallet);clusters.set(address,wallets);}
      const trending=this.trendingRows.get(chain)??[],fomoTokens=this.fomoTokens.get(chain)??[];
      const degen=this.degenRows.get(chain)??[];
      return [chain,{gmgn_tracked_wallets:(tracked.get(chain)??[]).length,gmgn_trending_rows:trending.length,gmgn_trending_quality_passed:trending.filter(row=>row.quality_passed===true).length,gmgn_trending_multiwindow_passed:trending.filter(row=>row.multiwindow_passed===true).length,degen_filtered_trending:degen.filter(row=>row.degen_sources?.includes("FILTERED TRENDING")).length,degen_microcaps:degen.filter(row=>row.is_microcap===true).length,degen_surge_events:degen.filter(row=>row.degen_signal_labels?.length).length,...(chain==="robinhood"?{pons_active_launches:this.ponsStats.active,pons_recent_graduated:this.ponsStats.graduated,pons_degen_candidates:this.ponsStats.qualified,pons_refreshed_at:this.ponsStats.refreshed_at}:{}),fomo_discovery_tokens:fomoTokens.length,fomo_trending_tokens:fomoTokens.filter(token=>token.sources.has("fomo_trending")).length,fomo_most_held_tokens:fomoTokens.filter(token=>token.sources.has("fomo_most_held")).length,fomo_eligible_holder_positions:fomoTokens.reduce((sum,token)=>sum+token.holders.length,0),fomo_tracked_profiles:this.fomoTrackedProfiles,fomo_profile_scan_errors:this.fomoProfileErrors,fomo_activity_refreshed_at:this.fomoActivityRefreshedAt?new Date(this.fomoActivityRefreshedAt).toISOString():null,fomo_recent_buy_swaps:fomoActivity.filter(row=>recent(row,envNumber("SIGNAL_LOOKBACK_SECONDS",1800))).length,fomo_recent_buy_wallets:new Set(fomoActivity.filter(row=>recent(row,envNumber("SIGNAL_LOOKBACK_SECONDS",1800))).map(row=>row.wallet).filter(Boolean)).size,fomo_tracked_buy_clusters:[...clusters.values()].filter(wallets=>wallets.size>=envNumber("MIN_TRACKED_BUY_WALLETS",3)).length}];
    }));
  }
  async latestTrendingAcross(chains:Chain[],limit=10):Promise<Json[]> {
    const requireStability=process.env.TRENDING_REQUIRE_MULTIWINDOW_STABILITY!=="false",rows:Json[]=chains.flatMap(chain=>(this.trendingRows.get(chain)??[]).map(row=>({...row,chain} as Json))).filter(row=>row.quality_passed===true&&(!requireStability||row.multiwindow_passed===true));
    return rows.sort((a,b)=>Number(b.multiwindow_score??0)-Number(a.multiwindow_score??0)||Number(b.volume_15m??0)-Number(a.volume_15m??0)||Number(b.volume??b.volume_5m??0)-Number(a.volume??a.volume_5m??0)).slice(0,limit);
  }
  ...
  async enrichDegenRows(rows:Json[]):Promise<Json[]> {
    const limit=envNumber("PONS_CARD_ENRICH_LIMIT",20),holderLimit=envNumber("PONS_CARD_HOLDER_LIMIT",5),output=rows.map(row=>({...row})),targets=output.filter(row=>row.chain==="robinhood"&&row.pons_status).slice(0,Math.max(0,limit));
    for(const row of targets){
      try {
        const address=String(row.address);if(!validTokenAddress("robinhood",address))continue;
        const info=await this.gmgn.tokenInfo("robinhood",address),holders=await this.gmgn.tokenHolders("robinhood",address,holderLimit),current=finiteNumber(info.price?.price),circulating=finiteNumber(info.circulating_supply),total=finiteNumber(info.max_supply??info.total_supply),athPrice=finiteNumber(info.ath_price),createdAt=finiteNumber(info.pool?.creation_timestamp??info.open_timestamp??info.creation_timestamp),price1h=finiteNumber(info.price?.price_1h);
  ...
  async monitorCallMultiples(chains:Chain[],displayedRows:Json[],now=Math.floor(Date.now()/1000),triggered?:Map<Chain,Set<string>>):Promise<Alert[]> {
    ...
    for(const chain of chains){
      ...
      for(const baseline of active){
        ...
        try{const snapshot=await this.evaluateToken(chain,address,knownInfo);alert.token_snapshot=snapshot;alert.token_score=snapshot.verdict.score;alert.token_passed=snapshot.verdict.passed;alert.token_warnings=snapshot.verdict.warnings;}catch(error){if(isRateLimit(error))console.warn("Milestone safety re-scan stopped because GMGN entered cooldown",String(error));else console.warn(`${chain}: milestone safety re-scan unavailable for ${address}`,String(error));}
        output.push(alert);
      }
  ...
  private candidate(map:Map<string,SignalCandidate>,chain:Chain,address:string,symbol?:string):SignalCandidate {const key=addressKey(address);let item=map.get(key);if(!item){item={chain,address,...(symbol?{symbol}:{}),sources:new Set(),sourceIds:new Set(),wallets:new Set(),buyWallets:new Set(),twitterAccounts:new Set(),firstTimestamp:Math.floor(Date.now()/1000),aggregateBuyUsd:0};map.set(key,item);}if(symbol&&!item.symbol)item.symbol=symbol;return item;}
  private addEvent(map:Map<string,SignalCandidate>,chain:Chain,row:Json,source:SignalSource):void {const address=String(row.base_address??row.token_address??row.token?.address??row.address??"");if(!validTokenAddress(chain,address)||!recent(row,envNumber("SIGNAL_LOOKBACK_SECONDS",1800)))return;const item=this.candidate(map,chain,address,row.base_token?.symbol??row.token?.symbol??row.symbol);item.sources.add(source);const wallet=addressKey(String(row.maker??row.wallet??row.wallet_address??""));if(wallet){item.wallets.add(wallet);item.buyWallets.add(wallet);}const id=String(row.transaction_hash??row.tx_hash??row.id??"");if(id)item.sourceIds.add(id);item.aggregateBuyUsd+=Number(row.amount_usd??row.cost_usd??0)||0;const ts=timestamp(row);if(ts)item.firstTimestamp=Math.min(item.firstTimestamp,ts);}

  private loadTrackedWallets():Map<Chain,string[]> {
    const output=new Map<Chain,string[]>(),dir=join(DATA_ROOT,"reports"),fomoPath=join(DATA_ROOT,"fomo","data","qualified-wallets.json");
    const files=existsSync(dir)?readdirSync(dir).filter(name=>/^daily-wallet-scan-.*\.json$/.test(name)&&!name.includes("cache")).sort().reverse():[];
    const latestGmgnFile=files.at(0),gmgnPath=latestGmgnFile?join(dir,latestGmgnFile):undefined;
    const paths=[gmgnPath,fomoPath,trackedWalletSeedPath].filter((path):path is string=>typeof path==="string").filter(path=>existsSync(path));
    const signature=`mongo:${this.mongoTrackedWalletRows.length}:`+paths.map(path=>`${path}:${statSync(path).mtimeMs}`).join("|");
    if(this.trackedWalletCache&&signature===this.trackedWalletSignature)return this.trackedWalletCache;
    if(gmgnPath)try {const report=JSON.parse(readFileSync(gmgnPath,"utf8"));for(const row of report.tracked_wallets??report.qualified_wallets??[]){const chain=row.chain as Chain,wallet=String(row.wallet??"");if(this.config.enabled_chains.includes(chain)&&wallet)output.set(chain,[...(output.get(chain)??[]),wallet]);}}
    catch(error){console.warn("Could not load GMGN qualified-wallet report",String(error));}
    if(existsSync(fomoPath)){try{const report=JSON.parse(readFileSync(fomoPath,"utf8"));for(const row of report.qualified_wallets??[]){if(row.verification_source!=="gmgn")continue;const chain=row.chain as Chain,wallet=String(row.wallet??"");if(this.config.enabled_chains.includes(chain)&&wallet&&!output.get(chain)?.includes(wallet))output.set(chain,[...(output.get(chain)??[]),wallet]);}}catch(error){console.warn("Could not load Fomo qualified-wallet report",String(error));}}
    for(const row of trackedWalletSeeds()){if(row.source!=="gmgn")continue;const chain=row.chain as Chain,wallet=String(row.wallet??"");if(this.config.enabled_chains.includes(chain)&&wallet&&!output.get(chain)?.includes(wallet))output.set(chain,[...(output.get(chain)??[]),wallet]);}
    for(const row of this.mongoTrackedWalletRows){if(row.source==="fomo"&&row.tracking_tier!=="qualified"&&row.verification_source!=="gmgn")continue;const chain=row.chain as Chain,wallet=String(row.wallet??"");if(this.config.enabled_chains.includes(chain)&&wallet&&!output.get(chain)?.includes(wallet))output.set(chain,[...(output.get(chain)??[]),wallet]);}
    this.trackedWalletSignature=signature;return this.trackedWalletCache=output;
  }
  invalidateTrackedWalletCache():void {this.trackedWalletCache=undefined;this.trackedWalletSignature="";}
  ...
  private async collectWalletSignals(chain:Chain,map:Map<string,SignalCandidate>):Promise<void> {
    ...
    for(const wallet of qualifiedWallets.slice(0,envNumber("TRACKED_WALLET_FALLBACK_LIMIT",25))){try{for(const row of await this.gmgn.walletActivity(chain,wallet,20))if(row.event_type==="buy")this.addEvent(map,chain,{...row,wallet},"tracked_wallet");}catch(error){if(isRateLimit(error))throw error;console.warn(`${chain}: could not scan tracked wallet ${wallet}`,String(error));}}
  }
  ...
  private async refreshFomo():Promise<void>{
    ...
    const qualifiedPath=join(DATA_ROOT,"fomo","data","qualified-wallets.json");
    try {const fileRows:Json[]=existsSync(qualifiedPath)?(()=>{const report=JSON.parse(readFileSync(qualifiedPath,"utf8"));return report.tracked_wallets??report.qualified_wallets??[];})():[],generatedRows:Json[]=fileRows.length?fileRows:this.mongoTrackedWalletRows.filter(row=>row.source==="fomo"),manualRows=trackedWalletSeeds().filter(row=>row.source==="fomo"),rows=[...new Map([...generatedRows,...manualRows].map(row=>[`${row.fomo_user_id}:${row.chain}`,row])).values()],walletByProfileChain=new Map(rows.map(row=>[`${row.fomo_user_id}:${row.chain}`,row.wallet])),profiles=[...new Map(rows.map(row=>[String(row.fomo_user_id??""),row])).values()].filter(row=>row.fomo_user_id).slice(0,envNumber("FOMO_TRACKED_PROFILES",1000)),activity:Json[]=[];if(!profiles.length)return;this.fomoTrackedProfiles=profiles.length;let successful=0,failed=0;await mapLimit(profiles,envNumber("FOMO_ACTIVITY_CONCURRENCY",4),async profile=>{try{const data=await this.fomo.userSwaps(String(profile.fomo_user_id));successful++;for(const swap of data.swaps??[]){if(!swap.outTradeId)continue;const chain=fomoChain(swap.outNetworkId??swap.networkId),wallet=walletByProfileChain.get(`${profile.fomo_user_id}:${chain}`),created=Math.floor(Date.parse(swap.createdAt??0)/1000),tokenAddress=String(swap.outTokenAddress??"");if(chain&&wallet&&validTokenAddress(chain,tokenAddress)&&created>=Math.floor(now/1000)-envNumber("SIGNAL_LOOKBACK_SECONDS",1800))activity.push({...swap,chain,wallet,tokenAddress,timestamp:created,amount_usd:Number(swap.humanUsdAmountIn??swap.humanUsdAmountOut??0),discovery_sources:profile.discovery_sources??[]});}}catch(error){failed++;console.warn(`Fomo activity unavailable for ${profile.fomo_handle??profile.fomo_user_id}`,String(error));}});this.fomoProfileErrors=failed;if(successful){this.fomoActivity.clear();for(const row of activity)this.fomoActivity.set(row.chain,[...(this.fomoActivity.get(row.chain)??[]),row]);this.fomoActivityRefreshedAt=Date.now();}}catch(error){console.warn("Fomo tracked-wallet activity scan failed",String(error));}
  }
  private attachFomo(chain:Chain,map:Map<string,SignalCandidate>):void{
    ...
    for(const trade of this.fomoActivity.get(chain)??[]){if(!recent(trade,envNumber("SIGNAL_LOOKBACK_SECONDS",1800)))continue;const item=this.candidate(map,chain,String(trade.tokenAddress)),wallet=addressKey(String(trade.wallet));item.sources.add("fomo_tracked_wallet");for(const source of trade.discovery_sources??[]){if(String(source).startsWith("fomo_leaderboard"))item.sources.add("fomo_leaderboard");if(String(source).startsWith("fomo_most_held"))item.sources.add("fomo_most_held");if(String(source).startsWith("fomo_trending"))item.sources.add("fomo_trending");}item.wallets.add(wallet);item.buyWallets.add(wallet);item.aggregateBuyUsd+=Number(trade.amount_usd??0);item.firstTimestamp=Math.min(item.firstTimestamp,Number(trade.timestamp));}
  }
  ...
  private async collectCandidates(chain:Chain,limit:number):Promise<Map<string,SignalCandidate>> {
    ...
    await this.collectWalletSignals(chain,map);this.attachFomo(chain,map);this.attachTwitter(map);return map;
  }
  ...
  async scan(chain:Chain,limit=200):Promise<Alert[]> {
    ...
    try {await Promise.all([this.refreshTwitter(),this.refreshFomo()]);const cfg=configForChain(this.config,chain),store=this.store(chain),candidates=await this.collectCandidates(chain,limit),output:Alert[]=[];const selected=[...candidates.values()].filter(candidate=>shouldInvestigate(candidate,envNumber("MIN_SIGNAL_STRENGTH",3))).sort((a,b)=>signalStrength(b)-signalStrength(a)).slice(0,envNumber("MAX_CANDIDATES_PER_CHAIN",5));
      for(const candidate of selected){const cooldown=envNumber("SIGNAL_ALERT_COOLDOWN_MS",21600000);if(Date.now()/1000-store.lastAlertAt(candidate.address)<cooldown/1000)continue;const [info,security,pool]=await Promise.all([this.gmgn.tokenInfo(chain,candidate.address),this.gmgn.tokenSecurity(chain,candidate.address),this.gmgn.tokenPool(chain,candidate.address)]);const currentPrice=number(info.price?.price,null),verdict=scoreToken(info,security,pool,{median_entry_price_usd:currentPrice,max_price_chase_ratio:cfg.cluster.max_price_chase_ratio},cfg.token);const failures=[...verdict.reasons.filter(r=>r.startsWith("FAIL ")),...verdict.warnings],catalysts=this.catalysts(cfg),catalyst=catalysts[`${chain}:${candidate.address}`]??catalysts[candidate.address];const tier:Alert["tier"]=failures.length||!verdict.passed?"REJECT":(catalyst||!cfg.require_catalyst_for_call?"CALL":"RESEARCH");const holders=tier==="REJECT"?[]:await this.tokenHolders(chain,candidate.address);const snapshot=buildTokenSnapshot(chain,candidate.address,info,security,pool,holders,verdict),market=candidate.market??{};const alert:Alert={tier,chain,address:candidate.address,...(snapshot.symbol?{symbol:snapshot.symbol}:candidate.symbol?{symbol:candidate.symbol}:{}),name:snapshot.name,wallet_count:candidate.wallets.size,tracked_buy_wallet_count:candidate.buyWallets.size,independent_funders:candidate.wallets.size,aggregate_buy_usd:Math.round(candidate.aggregateBuyUsd*100)/100,median_buy_usd:0,window_seconds:Math.max(0,Math.floor(Date.now()/1000)-candidate.firstTimestamp),wallets:[...candidate.wallets],sources:[...candidate.sources],signal_strength:signalStrength(candidate),twitter_accounts:[...candidate.twitterAccounts],market_cap:snapshot.marketCap??market.market_cap,liquidity:snapshot.liquidity??market.liquidity,price_change_5m:market.price_change_percent5m??market.price_change_5m,token_score:verdict.score,token_reasons:verdict.reasons,warnings:verdict.warnings,failures,catalyst,token_snapshot:snapshot,invalidation:"Downgrade if tracked wallets distribute, liquidity falls below threshold, momentum reverses, or contract safety changes."};if(store.saveAlert(alert,candidate.firstTimestamp))output.push(alert);}
      const richAlerts=new Set(output.filter(alert=>alert.tier!=="REJECT").map(alert=>addressKey(alert.address)));
      for(const candidate of candidates.values())if(hasTrackedBuyCluster(candidate,envNumber("MIN_TRACKED_BUY_WALLETS",3))&&!richAlerts.has(addressKey(candidate.address))){const cooldown=envNumber("WALLET_CLUSTER_ALERT_COOLDOWN_MS",1800000);if(Date.now()/1000-store.lastAlertAt(candidate.address,"WALLET_CLUSTER")<cooldown/1000)continue;const alert:Alert={tier:"RESEARCH",kind:"WALLET_CLUSTER",cluster_strength:candidate.buyWallets.size>=4?"STRONG":"SIGNAL",chain,address:candidate.address,...(candidate.symbol?{symbol:candidate.symbol}:{}),wallet_count:candidate.buyWallets.size,wallets:[...candidate.buyWallets],sources:[...candidate.sources],aggregate_buy_usd:Math.round(candidate.aggregateBuyUsd*100)/100,first_timestamp:candidate.firstTimestamp};if(store.saveAlert(alert,candidate.firstTimestamp))output.push(alert);}
      return output;
