...
const bool = (value: unknown): boolean | undefined =>
  value === true || [1, "1", "true", "yes"].includes(value as any)
    ? true
    : value === false || [0, "0", "false", "no"].includes(value as any)
      ? false
      : undefined;
...
const ratioChange = (current?: number, previous?: number): number | undefined =>
  current !== undefined && previous !== undefined && previous > 0
    ? (current / previous - 1) * 100
    : undefined;
const positiveTimestamp = (...values: unknown[]): number | undefined => {
  for (const value of values) {
    const parsed = definedNumber(value);
...
export function buildTokenSnapshot(
  ...
  const marketCap =
    price !== undefined && circulating !== undefined ? price * circulating : undefined;
  const fdv = price !== undefined && total !== undefined ? price * total : undefined;
  const createdAt = positiveTimestamp(
    pool.creation_timestamp,
    info.pool?.creation_timestamp,
    info.open_timestamp,
    info.creation_timestamp,
  );
  const twitterUsername = String(info.link?.twitter_username ?? "")
    .trim()
    .replace(/^@/, "");
  const twitter =
    httpUrl(info.link?.twitter) ??
    (twitterUsername ? `https://x.com/${encodeURIComponent(twitterUsername)}` : undefined);
  return {
    chain,
    address,
    ...
    poolAddress:
      String(pool.pool_address ?? info.biggest_pool_address ?? info.pool?.pool_address ?? "") ||
      undefined,
    website: httpUrl(info.link?.website),
    twitter,
    telegram: httpUrl(info.link?.telegram),
    gmgn: httpUrl(info.link?.gmgn),
    ...
    buyTax: definedNumber(security.buy_tax),
    sellTax: definedNumber(security.sell_tax),
    topHolders: holders
      .map(holderSnapshot)
      .filter((holder): holder is TokenHolderSnapshot => Boolean(holder))
      .slice(0, 5),
    verdict,
  };
}
