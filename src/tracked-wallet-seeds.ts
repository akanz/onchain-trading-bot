...
export function parseTrackedWalletSeeds(payload: unknown): Json[] {
  ...
  if (!Array.isArray(rows))
    throw new Error("tracked-wallet-seeds.json must contain a wallets array");
  const seen = new Set<string>();
  return rows.map((value, index) => {
    if (!value || typeof value !== "object")
      throw new Error(`tracked wallet row ${index + 1} must be an object`);
    const row = value as Json,
      chain = String(row.chain ?? "") as Chain,
      source = String(row.source ?? ""),
      rawWallet = String(row.wallet ?? "");
    if (!CHAINS.has(chain)) throw new Error(`tracked wallet row ${index + 1} has an invalid chain`);
    if (!SOURCES.has(source))
      throw new Error(`tracked wallet row ${index + 1} has an invalid source`);
    const validWallet = chain === "sol" ? SOL_ADDRESS.test(rawWallet) : EVM_ADDRESS.test(rawWallet);
    if (!validWallet)
      throw new Error(`tracked wallet row ${index + 1} has an invalid wallet address`);
    if (source === "fomo" && !String(row.fomo_user_id ?? ""))
      throw new Error(`tracked Fomo row ${index + 1} is missing fomo_user_id`);
    const wallet = chain === "sol" ? rawWallet : rawWallet.toLowerCase(),
      key = `${source}:${chain}:${wallet}`;
