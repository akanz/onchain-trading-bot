...
import { buildDegenRows } from "../src/degen.js";
...
test("Pons cards are enriched from documented GMGN token and holder fields",async()=>{const gmgn={tokenInfo:async()=>({name:"Printer",symbol:"BRRR",circulating_supply:"1000000000",max_supply:"1000000000",ath_price:"0.00009",liquidity:"8000",holder_count:300,creation_timestamp:1_999_000,price:{price:"0.00004",price_1h:"0.00005",volume_1h:"120000",volume_24h:"240000",buys_1h:100,sells_1h:80},pool:{pool_address:"0xpool",exchange:"pons_v2"},stat:{top_10_holder_rate:.2,fresh_wallet_rate:.1},wallet_tags_stat:{smart_wallets:3},link:{gmgn:"https://gmgn.ai/token"}}),tokenHolders:async()=>[{address:address(999),amount_percentage:.08,usd_value:3200,tags:["smart_degen"]}]};const service=new TrackerService({enabled_chains:["robinhood"],default_chain:"robinhood"} as any,gmgn as any),[row]=await service.enrichDegenRows([{chain:"robinhood",address:address(998),pons_status:"ACTIVE",market_cap:1}]);assert.equal(row?.market_cap,40_000);assert.equal(row?.ath_market_cap,90_000);assert.ok(Math.abs(Number(row?.price_change_1h)+20)<1e-9);assert.equal(row?.top_holders[0].amount_percentage,.08);});
