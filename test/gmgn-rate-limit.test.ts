...
import { GmgnCooldownError, GmgnRateGate, gmgnRequestWeight, parseRateLimitReset } from "../src/gmgn-rate-limit.js";
...
test("weighted gate serializes bursts and waits for enough units",async()=>{
  let now=0;
  const waits:number[]=[];
  const gate=new GmgnRateGate({ratePerSecond:10,capacity:5,now:()=>now,sleep:async ms=>{waits.push(ms);now+=ms;}});
  const order:string[]=[];
  await Promise.all([
    gate.schedule(5,async()=>{order.push("heavy");}),
    gate.schedule(1,async()=>{order.push("light");}),
  ]);
...
test("a ban cancels already queued calls and blocks new calls without executing them",async()=>{
  ...
  const gate=new GmgnRateGate({ratePerSecond:10,capacity:5,now:()=>now,sleep:async ms=>{now+=ms;}});
  const first=gate.schedule(1,async()=>{calls++;gate.blockUntil(now+30_000);throw new Error("429");});
  const queued=gate.schedule(1,async()=>{calls++;});
