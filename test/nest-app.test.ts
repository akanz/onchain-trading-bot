...
test("NestJS resolves the API controller through dependency injection", async () => {
  const runtime = {
    config: { default_chain: "sol", enabled_chains: ["sol", "bsc", "robinhood"] },
    scheduledChains: ["sol", "bsc", "robinhood"],
    tracker: { roster: () => [], alerts: () => [] },
    botStore: {},
  };
  const module = await Test.createTestingModule({
    controllers: [AppController],
    providers: [AlertStream, { provide: RuntimeService, useValue: runtime }],
  }).compile();
  try {
