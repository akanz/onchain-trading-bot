...
test("Wilson bound shrinks small samples", () => {
  ...
  assert.ok(wilsonLowerBound(70, 100) > 0.59);
});

test("cluster requires distinct wallets", () => {
  const events = ["a", "a", "b", "c"].map((maker, i) => ({
    side: "buy",
    is_open_or_close: 0,
    base_address: "token",
    maker,
...
test("EVM honeypot is rejected", () => {
  ...
  assert.equal(
    scoreToken(
      ...
      security,
      { liquidity: "150000" },
      { median_entry_price_usd: 0.001, max_price_chase_ratio: 0.15 },
      base.token,
    ).passed,
    false,
  );
...
test("tracked-buy safety rejects all-zero holder analytics and thin liquidity", () => {
  const robinhood = configForChain(loadConfig(), "robinhood"),
    info = {
      symbol: "memestock",
      circulating_supply: "1000000000",
      holder_count: 4,
      liquidity: "803.2153",
      price: { price: "0.0001804965" },
      stat: {
  ...
  assert.ok(
    verdict.reasons.some((reason) => reason.includes("liquidity-to-market-cap ratio 0.45%")),
  );
  assert.ok(verdict.reasons.some((reason) => reason.startsWith("FAIL GMGN holder analysis")));
  assert.ok(
    verdict.reasons.some((reason) => reason.startsWith("FAIL top-10 concentration is populated")),
  );
});
...
test("tracked-buy safety accepts a complete liquid non-honeypot", () => {
  ...
  assert.equal(
    screenTrackedBuyToken(info, security, { liquidity: "100000" }, robinhood.token).passed,
    true,
  );
  assert.equal(
    screenTrackedBuyToken(
...
test("tracked-buy safety does not apply mature CALL thresholds to early tokens", () => {
  const cfg = configForChain(loadConfig(), "robinhood").token,
    security = {
      is_honeypot: false,
      is_blacklist: false,
      can_not_sell: 0,
    ...
    token = (symbol: string, marketCap: number, liquidity: number, holders: number) => ({
      info: {
        symbol,
        circulating_supply: "1000000",
        holder_count: holders,
        price: { price: String(marketCap / 1_000_000) },
        stat: { top_10_holder_rate: "0.2", fresh_wallet_rate: "0.1" },
      },
      pool: { liquidity: String(liquidity) },
    });
  for (const [symbol, marketCap, liquidity, holders] of [
    ["DTF", 1_500_000, 56_700, 500],
    ["Motion", 80_293, 26_073, 500],
    ["CHUD", 9_290, 7_048, 147],
  ] as const) {
    const row = token(symbol, marketCap, liquidity, holders);
    assert.equal(
      screenTrackedBuyToken(row.info, security, row.pool, cfg).passed,
      true,
      `${symbol} should remain observable`,
    );
  }
  const belowFloor = token("VOIDMINER", 3_504, 533, 300);
  assert.equal(
    screenTrackedBuyToken(belowFloor.info, security, belowFloor.pool, cfg).passed,
    false,
  );
  const contradictory = token("BROKEN", 5_000, 100_000, 300);
  assert.equal(
    screenTrackedBuyToken(contradictory.info, security, contradictory.pool, cfg).passed,
    false,
  );
});
...
test("contract lookup resolves every exact enabled-chain match", async () => {
  ...
  const gmgn = {
    tokenInfo: async (chain: string) =>
      chain === "base"
        ? { symbol: "TEST", standard: "erc20" }
        : chain === "bsc"
          ? { name: "Test", price: { address } }
          : { symbol: "", name: "", standard: "", price: { address: "" } },
  };
...
test("Solana address resolves without a cross-chain probe", async () => {
  ...
  assert.deepEqual(
    await service.resolveTokenChains("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
    ["sol"],
  );
});
