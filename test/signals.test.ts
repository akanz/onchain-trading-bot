...
test("early surge discovery admits a complete liquid microcap but rejects absurd market data",()=>{const earlyCfg={...cfg,min_liquidity_to_market_cap_ratio:.05,tracked_alert_min_liquidity_usd:500,tracked_alert_min_liquidity_to_market_cap_ratio:.01,tracked_alert_max_liquidity_to_market_cap_ratio:10,tracked_alert_min_market_cap_usd:5000,tracked_alert_min_holders:10,tracked_alert_max_top_10_holder_rate:.3},row={market_cap:70_000,liquidity:19_000,holder_count:722,top_10_holder_rate:.17,rug_ratio:.1,is_wash_trading:false};assert.equal(passesMarketGate(row,earlyCfg).passed,false);assert.equal(passesSurgeDiscoveryGate(row,earlyCfg).passed,true);assert.equal(passesSurgeDiscoveryGate({...row,top_10_holder_rate:0},earlyCfg).passed,false);assert.equal(passesSurgeDiscoveryGate({...row,top_10_holder_rate:.4},earlyCfg).passed,false);assert.equal(passesSurgeDiscoveryGate({...row,market_cap:1},earlyCfg).passed,false);assert.equal(passesSurgeDiscoveryGate({...row,market_cap:5000,liquidity:100_000},earlyCfg).passed,false);});
test("catastrophic market-cap collapse means more than a 99% loss",()=>{assert.equal(isCatastrophicMarketCapCollapse(100_000,1_000),false);assert.equal(isCatastrophicMarketCapCollapse(100_000,999),true);assert.equal(isCatastrophicMarketCapCollapse(undefined,1),false);});
...
test("tracked-wallet sells produce an exit observation without becoming a buy signal",async()=>{const address="0x8888888888888888888888888888888888888888",info={symbol:"EXIT",circulating_supply:1_000_000,holder_count:1000,price:{price:.2},stat:{top_10_holder_rate:.2}},security={is_honeypot:false,is_blacklist:false,can_not_sell:0,is_open_source:true,is_renounced:true,buy_tax:0,sell_tax:0,top_10_holder_rate:.2,lock_summary:{is_locked:true}},gmgn={tokenInfo:async()=>info,tokenSecurity:async()=>security,tokenPool:async()=>({liquidity:20_000}),tokenHolders:async()=>[]},service=new TrackerService({...loadConfig(),default_chain:"robinhood",enabled_chains:["robinhood"]} as any,gmgn as any),now=Math.floor(Date.now()/1000);(service as any).refreshTwitter=async()=>{};(service as any).refreshFomo=async()=>{};(service as any).collectCandidates=async()=>{const candidate:any={chain:"robinhood",address,sources:new Set(),sellSources:new Set(["fomo_tracked_wallet"]),sourceIds:new Set(),wallets:new Set(),buyWallets:new Set(),sellWallets:new Set(["wallet-a"]),traderLabels:new Set(),sellTraderLabels:new Set(["traderA"]),twitterAccounts:new Set(),firstTimestamp:now-5,aggregateBuyUsd:0,aggregateSellUsd:900};return new Map([[address,candidate]]);};const [alert]=await service.scan("robinhood");assert.equal(alert?.kind,"TRACKED_WALLET_SELL");assert.equal(alert?.tracking_label,"SELL WATCH");assert.deepEqual(alert?.traders,["traderA"]);assert.equal(alert?.aggregate_sell_usd,900);assert.equal(alert?.tracked_buy_wallet_count,undefined);await service.close();});
