...
import { formatAlert, formatDegenDigest, formatSuppressedTokens, formatTokenCard, formatTrackingStatus, formatTrendingDigest } from "../src/format.js";
...
test("tracking status exposes recent Fomo swaps, multiplier baselines, polling health, and safety suppressions",()=>{const message=formatTrackingStatus(["sol"],{sol:{gmgn_tracked_wallets:4,fomo_tracked_profiles:33,multiplier_monitor:{active_baselines:9,crossed_unannounced:1,max_observed_multiple:2.76},fomo_recent_buy_swaps:1,fomo_recent_buy_wallets:1,fomo_profile_scan_errors:0,tracked_buy_safety:{checked:2,passed:1,suppressed:1,unavailable:0},fomo_recent_buys:[{trader:"KaminaSOL",token:"2beEEfXFh1gqHvjaDTsxF5oowdNMskg997tgogk4pump",amount_usd:497.503,bought_at:"2026-08-30T13:03:41.000Z"}]}});assert.match(message,/Monitored addresses: 4 · Fomo-native profiles: 33/);assert.match(message,/9 active · 1 crossed but unannounced · max 2\.76×/);assert.match(message,/KaminaSOL/);assert.match(message,/\$497\.5/);assert.match(message,/profile errors: 0/);assert.match(message,/Safety checks: 2 · passed: 1 · suppressed: 1 · unavailable: 0/);});
