...
const candles = (closes: number[], volumes: number[]) =>
...
test("persistent volume and stable structure can qualify while price is mildly red", () => {
  const rows = candles(
    [1, 0.998, 0.996, 0.994, 0.992, 0.99, 0.989, 0.988, 0.987, 0.986, 0.985, 0.984, 0.983],
    [18000, 19000, 21000, 22000, 24000, 26000, 23000, 25000, 27000, 26000, 28000, 29000, 30000],
  );
  const result = analyzeTrendingCandles(
    rows,
    {
      volume: 30000,
      liquidity: 250000,
...
test("a vertical pump with one concentrated volume spike does not pass as stable", () => {
  const rows = candles(
    [1, 1, 1.01, 1.01, 1.02, 1.02, 1.03, 1.04, 1.04, 1.05, 1.06, 1.07, 1.8],
    [5000, 5000, 5500, 5000, 5200, 5100, 5000, 5500, 5200, 5100, 5300, 5000, 300000],
  );
  const result = analyzeTrendingCandles(
    rows,
    { volume: 300000, liquidity: 100000, market_cap: 300000, buys: 100, sells: 12 },
    25000,
  );
...
test("a breakdown remains a hard rejection even with sufficient volume", () => {
  const rows = candles(
    [1, 0.98, 0.95, 0.92, 0.88, 0.8, 0.72, 0.65, 0.58, 0.52, 0.48, 0.44, 0.4],
    Array(13).fill(30000),
  );
  const result = analyzeTrendingCandles(
    rows,
    { volume: 30000, liquidity: 300000, market_cap: 400000, buys: 20, sells: 50 },
    25000,
  );
